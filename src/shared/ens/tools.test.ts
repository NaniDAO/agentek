...
describe("ENS Tools", () => {
    describe("resolveENSTool", () => {
        it("should resolve ENS name to address", async () => {
            const address = await resolveENSTool.execute(mockClient, { name: "vitalik.eth" });
    ...
    describe("resolveENSTool", () => {
        it("should resolve DNSSEC+ENS name to address", async () => {
            const address = await resolveENSTool.execute(mockClient, { name: "namesys.xyz" });
    ...
    describe("resolveENSTool", () => {
        it("should resolve Emoji ENS name to address", async () => {
            const address = await resolveENSTool.execute(mockClient, { name: "👨‍🍳.eth" });
    ...
    describe("lookupENSTool", () => {
        it("should lookup ENS address to name", async () => {
            const name = await lookupENSTool.execute(mockClient, { address: "0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045" });
    ...
    describe("lookupENSTool", () => {
        it("should lookup ENS address to name", async () => {
            const name = await lookupENSTool.execute(mockClient, { address: "0x087d16Aa5F00F39b87BBeaEFB4124006D2371eAb" });
